...
RayTracer::HitRecord RayTracer::Cylinder::hit(const Maths::Ray& ray) const
{
    ...
    double a = pow(ray._direction._x, 2) + pow(ray._direction._z, 2);
    ...
    if (discriminant == 0) {
        ...
        if (distance < 0)
            return hit_record;
        if (hit_record.getIntersectionPoint()._y < this->getPosition()._y || hit_record.getIntersectionPoint()._y > this->getPosition()._y + _height)
            return hit_record;
